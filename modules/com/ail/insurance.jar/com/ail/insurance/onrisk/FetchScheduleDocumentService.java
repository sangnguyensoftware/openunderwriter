...
import com.ail.insurance.onrisk.GenerateScheduleDocumentService.GenerateScheduleDocumentCommand;
import com.ail.insurance.policy.Policy;

@ServiceImplementation
public class FetchScheduleDocumentService extends Service<FetchDocumentArgument> {
   ...
   /**
    ...
    public void invoke() throws BaseException {
        ...
        // We only generate wording docs on demand, so if there isn't one -
        // generate it.
        if (policy.retrieveScheduleDocument() == null) {
            GenerateScheduleDocumentCommand cmd = getCore().newCommand(GenerateScheduleDocumentCommand.class);
            cmd.setPolicyArg(policy);
            cmd.invoke();
            policy.attachScheduleDocument(cmd.getDocumentRet());
            policy = getCore().update(policy);
            args.setModelRet(policy);
        }

        args.setDocumentRet(policy.retrieveScheduleDocument());
    }
}

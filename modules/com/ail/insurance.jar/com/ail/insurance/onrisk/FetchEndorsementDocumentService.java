...
import com.ail.insurance.onrisk.GenerateEndorsementDocumentService.GenerateEndorsementDocumentCommand;
import com.ail.insurance.policy.Policy;

@ServiceImplementation
public class FetchEndorsementDocumentService extends Service<FetchDocumentArgument> {
    ...
    public void invoke() throws BaseException {
        ...
        // Fetch the saved policy from persistence
        ...
        // We only generate Endorsement docs on demand, so if there isn't one - generate it.
        if (policy.retrieveEndorsementDocument() == null) {
            GenerateEndorsementDocumentCommand cmd = getCore().newCommand(GenerateEndorsementDocumentCommand.class);
            cmd.setPolicyArg(policy);
            cmd.invoke();
            policy.attachEndorsementDocument(cmd.getDocumentRet());
            policy = getCore().update(policy);
            args.setModelRet(policy);
        }

        args.setDocumentRet(policy.retrieveEndorsementDocument());
    }
}

...
import com.ail.insurance.payment.PremiumPaymentProcessorService.PremiumPaymentProcessorArgument;
import com.ail.insurance.policy.Policy;
import com.ail.party.Party;

/**
 * <p>
 * This service searches for scheduled payments that are due on the effective
 * date specified and debits the policy holder's account. Matching credits are
 * made to the accounts defined by the product's rules - this typically includes
 * premium payments to the product owner's account and any commission payments
...
public class PremiumPaymentProcessorService extends Service<PremiumPaymentProcessorArgument> {
    ...
    public void invoke() throws BaseException {
        ...
        for (MoneyProvision payment : payments) {
            if (journalExistsForPayment(payment, PREMIUM_DUE, args.getFromDateArg(), args.getToDateArg())) {
                continue;
            }

            Policy policy = fetchPolicyForMoneyProvision(payment);

            Set<JournalLine> journalLines = buildJournalLines(policy, payment, PREMIUM, policy.getClient(), args.getPaymentReceiverArg());

            Journal journal = buildJournal(policy, journalLines, payment, args.getToDateArg(), PREMIUM_DUE);
    ...
    @ServiceCommand(defaultServiceClass = PremiumPaymentProcessorService.class)
    public interface PremiumPaymentProcessorCommand extends Command, PremiumPaymentProcessorArgument {
    }

    @ServiceArgument
    public interface PremiumPaymentProcessorArgument extends Argument {
        ...
        /**
         * The destination of the Client payment
         */
        Party getPaymentReceiverArg();

        /**
         * The destination of the Client payment
         */
        void setPaymentReceiverArg(Party paymentReceiverArg);

...
public class NotifyBrokerByPubNubService extends Service<NotifyBrokerByPubNubArgument> {
    ...
    @ServiceArgument
    public interface NotifyBrokerByPubNubArgument extends Argument {
        Long getPolicyIdArg();
    ...
    @ServiceCommand(defaultServiceClass = NotifyBrokerByPubNubService.class)
    public interface NotifyBrokerByPubNubCommand extends Command, NotifyBrokerByPubNubArgument {
    }
    ...
    public void invoke() throws PreconditionException, BaseException {
        ...
        Broker broker = determineBrokerParty(policy);

        sendNotification(policy, broker);
    }
    ...
    Broker determineBrokerParty(Policy policy) throws PreconditionException {
        if (policy.getBroker() == null) {
        	throw new PreconditionException("policy.getBroker() == null");
        }
        return policy.getBroker();
    }

    void sendNotification(Policy policy, Broker broker) throws BaseException {
        String channel = (String) broker.xpathGet(PARTY_PUBNUB_CHANNEL_XPATH);

        if (channel == null) {
            throw new PreconditionException("broker.xpathGet('" + PARTY_PUBNUB_CHANNEL_XPATH + "') == null");
        }

        String message = args.getMessageArg();

        if (message == null || message.isEmpty()) {
            List<Attribute> messages = broker.getAttribute();

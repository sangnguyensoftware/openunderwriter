 ...
 * who either own the object, or are in the same organisation as the owner, or
 * who are members of an administrative role. The name of the administrative
 * role is defined as a service configuration parameter.
 */
@ServiceImplementation
public class OwnerOrganisationAndAdministratorAccessibilityService extends Service<ConfirmObjectAccessibilityToUserArgument> {
    ...
    public void invoke() throws PreconditionException, UpdateException {
        ...
        if (objectIsOwnedBy(userId) || objectsOwnerIsInSameOrganisationAs(userId) || userHasOverrideRole(userId)) {
            args.setReadAccessRet(true);
            args.setWriteAccessRet(true);
        }
    }
    ...
    private boolean objectsOwnerIsInSameOrganisationAs(Long userId) throws PreconditionException {
        try {
            if (isObjectOwnedByGuest()) {
                return false;
            }

            User owner = UserLocalServiceUtil.getUser(args.getObjectArg().getOwningUser());
            ...
            throw new PreconditionException("Organization could not be queried for user: "+userId, e);
        }

...
public class FetchCertificateDocumentService extends Service<FetchDocumentArgument> {
    ...
    public void invoke() throws BaseException {
        ...
        // We only generate certificate docs on demand, so if there isn't one - generate it.
        if (policy.retrieveCertificateDocument() == null) {
            ...
            cmd.invoke();
            policy.attachCertificateDocument(cmd.getDocumentRet());
            policy = getCore().update(policy);
            args.setModelRet(policy);
        }

        args.setDocumentRet(policy.retrieveCertificateDocument());
    }
}
